...
- **v10.9.0 `variant_key` migration** (`2026_08_07_100000_ExperimentMeasurementsVariantKeyUnique`): run it preferably quiesced in the hosts (brief `php artisan down` or maintenance window). Un-quiesced it cannot corrupt data (old unique index stays until the new one is verified — same in `down()` —, merge groups run in lockForUpdate transactions, exposure counting is an atomic relative increment that re-targets the canonical row); worst case is one lost exposure increment. `variant_key` is the injective stand-in for `variant` in the unique index ('' = NULL, `'v'.value` otherwise), because NULLs never conflict in a unique index. It is a **database-generated VIRTUAL column — never write it from PHP**; the DB deriving it keeps even pre-v10.9 writers during a rolling deploy correct by construction (VIRTUAL because SQLite cannot ALTER-ADD a STORED generated column; MySQL and SQLite both index virtual columns).
